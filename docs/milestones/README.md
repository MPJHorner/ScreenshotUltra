# Milestones

The original design document lives in [`plan.md`](../../plan.md). This
directory tracks delivery: one file per milestone, each updated as the work
lands.

| Milestone | Title              | Status        |
|-----------|--------------------|---------------|
| [M1](M1-press-the-key.md)        | Press the key       | ✅ shipped (v0.1.0) |
| [M2](M2-annotate.md)             | Annotate            | 🚧 in progress (Quick Tray shipped) |
| [M3](M3-record.md)               | Record              | ⏳ planned    |
| [M4](M4-scroll-and-beautify.md)  | Scroll & Beautify   | ⏳ planned    |
| [M5](M5-ocr-and-polish.md)       | OCR & polish        | ⏳ planned    |
| [M6](M6-ship.md)                 | Ship v1.0           | ⏳ planned    |

Mark milestone files when work begins (⏳ → 🚧) and again when it ships
(🚧 → ✅).
